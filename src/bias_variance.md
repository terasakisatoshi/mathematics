...
- バイアスは理想的な $$\mathbb{E}[y_t|\boldsymbol{x}]$$ と様々なデータを用いて学習させたモデルの出力値とのずれを表し、手法自体のズレを表す.

- バリアンスは特定のデータで得たモデル $$y=y(\boldsymbol{x})$$ と全てのデータのとりかたについての予測値とのズレの平均を表す.
...
頻度主義に基づいた立場ではバイアス・バリアンス分解によってモデルの複雑さの問題への解釈を得る。とはいえ、バイアス・バリアンス分解がデータの選択の平均によって展開される議論のため, 実際問題1組の観測データしか与えられていない現実的な場面では有用性が限られる（そりゃそうだ・・・）.
そもそも、それなりのサイズを持つ独立な学習データがあればバイアス・バリアンす分解云々を議論するよりそれらをまとめて一つの学習データとし、それを学習させればいいのではという気持ちになる.
